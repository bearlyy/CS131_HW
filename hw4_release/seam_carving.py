...
def energy_function(image):
    ...
    dy, dx = np.gradient(gray)  # 偏导计算
    out = np.fabs(dy) + np.fabs(dx)
    ### END YOUR CODE
...
def compute_cost(image, energy, axis=1):
    ...
    ### YOUR CODE HERE
    v_find_min = np.vectorize(find_min)
    for i in range(1,H):
        row1, row, row2 = triplicate(cost[i-1])
        min_cost, path = v_find_min(row1, row, row2)
...
def remove_seam(image, seam):
    ...
    H, W, C = image.shape
    ### YOUR CODE HERE
    out = image.reshape(H*W,C)
    seam = W*np.arange(0,H) + seam
    out = np.delete(out, seam, axis = 0)
...
def duplicate_seam(image, seam):
    ...
    for i in range(H):
        index = seam[i]
        out[i,:index+1,:] = image[i,:index+1,:]
        out[i,index+1:,:] = image[i,index:,:]
    ### END YOUR CODE
...
def find_seams(image, k, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(k):
        ...
        a=np.arange(H).astype(int)
        b=seam.astype(int)
        c=indices[a, b].astype(int)
        assert np.all(seams[a, c] == 0), \
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(k):
        #np.where(seams == i+1)返回包含值i+1位置的元组
        #np.where(seams == i+1)[0]返回包含值i+1位置的行
        #np.where(seams == i+1)[1]返回包含值i+1位置的列
        seam = np.where(seams == i+1)[1]
        out = duplicate_seam(out, seam) # 复制第i条缝
        # 在seams上复制第i条缝，调整seams位置
        Hs, Ws = seams.shape
        seams = seams.reshape(Hs*Ws)
        seam = Ws*np.arange(0,Hs) + seam
        seams = np.insert(seams, seam, seams[seam])
        seams = seams.reshape(Hs, (Ws + 1))
    ### END YOUR CODE
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1,H):
        for j in range(W):
            if (j == 0):
                cost[i,j]=energy[i,j]+min([cost[i-1,j],\
                                           cost[i-1,j+1]+np.abs(image[i,j+1]-image[i-1,j])])
                paths[i][j] = np.argmin([cost[i-1,j],\
                                         cost[i-1,j+1]+np.abs(image[i,j+1]-image[i-1,j])])
            elif(j == W-1):
                cost[i,j]=energy[i,j]+min([cost[i-1,j-1]+np.abs(image[i-1,j]-image[i,j-1]),\
                                           cost[i-1,j]])
                paths[i][j] = np.argmin([cost[i-1,j-1]+np.abs(image[i-1,j]-image[i,j-1]),\
                                         cost[i-1,j]]) - 1
            else:
                cost[i,j]=energy[i,j]+np.abs(image[i,j+1]-image[i,j-1])+\
                                             min([cost[i-1,j-1]+np.abs(image[i-1,j]-image[i,j-1]),\
                                                  cost[i-1,j],cost[i-1,j+1]+np.abs(image[i,j+1]-image[i-1,j])])
                paths[i][j] = np.argmin([cost[i-1,j-1]+np.abs(image[i-1,j]-image[i,j-1]),\
                                         cost[i-1,j],cost[i-1,j+1]+np.abs(image[i,j+1]-image[i-1,j])]) - 1
    ### END YOUR CODE
