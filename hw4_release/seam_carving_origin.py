...
def compute_cost(image, energy, axis=1):
    ...
    for i in range(1, H):
        for j in range(W):
            if j == 0:
                cost[i, j] = energy[i, j] + np.min(cost[i - 1, j:j + 2])
                paths[i, j] = np.argmin(cost[i - 1, j:j + 2])
            elif j == W - 1:
                cost[i, j] = energy[i, j] + np.min(cost[i - 1, j - 1:j + 1])
                paths[i, j] = np.argmin(cost[i - 1, j - 1:j + 1]) -1
            else:
                cost[i, j] = energy[i, j] + np.min(cost[i - 1, j - 1:j + 2])
...
def remove_seam(image, seam):
    ...
    for i in range(H):
        index = seam[i]
        out[i,:index,:] = image[i,:index,:]
        out[i,index:,:] = image[i,index+1:,:]
    ### END YOUR CODE
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    ### YOUR CODE HERE
    k = size-W
    add_space = np.zeros([H,k,C])
    out = np.concatenate((out,add_space),axis=1)
    seams = find_seams(image, k)
    for i in range(H):
        j = W - 1
        while j >= 0:
            if seams[i,j] != 0:
                out[i,j+1:,:] = out[i,j:-1,:].copy()
            j -= 1
    ### END YOUR CODE
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1, H):
        for j in range(W):
            if j == 0:
                cost[i, j] = np.min([cost[i-1,j] + np.abs(image[i-1,j]-image[i,j+1]),
                                     cost[i-1,j+1]])
                paths[i, j] = np.argmin(cost[i - 1, j:j + 2])
            elif j == W - 1:
                cost[i, j] = np.min([cost[i-1,j-1] + np.abs(image[i-1,j]-image[i,j-1]),
                                    cost[i-1,j]])
                paths[i, j] = np.argmin(cost[i - 1, j - 1:j + 1]) -1
            else:
                CL = cost[i-1,j-1] + np.abs(image[i,j+1]-image[i,j-1]) + np.abs(image[i-1,j]-image[i,j-1])
                CR = cost[i-1,j] + np.abs(image[i,j+1]-image[i,j-1]) + np.abs(image[i-1,j]-image[i,j+1])
                CV = cost[i-1,j+1] + np.abs(image[i,j+1]-image[i,j-1])
                cost[i, j] = np.min(np.array([CR, CL, CV]))
                paths[i, j] = np.argmin(cost[i - 1, j - 1:j + 2]) - 1
    ### END YOUR CODE
