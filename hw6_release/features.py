...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        if method == 'svd':
            vecs, _ = self._svd(X_centered)
        elif method == 'eigen':
            vecs, _ = self._eigen_decomp(X_centered)
        self.W_pca = vecs
        # END YOUR CODE
    ...
    def _eigen_decomp(self, X):
        ...
        e_vals = e_vals[sorted_index]
        e_vecs = e_vecs[:,sorted_index]
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        X_centered = np.dot(X_proj,np.linalg.pinv(self.W_pca[:,:n_components]))
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            S_i = np.dot(X_i_centered.T,X_i_centered)
            scatter_within += S_i
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            # replaced every example labeled i with mu_i.
            X_replaced[y==i] = mu_i
        # the covariance matrix of X_replaced
        scatter_between = np.dot(X_replaced.T, X_replaced)
            # END YOUR CODE
