...
def compute_distances(X1, X2):
    ...
    dists = np.sqrt(-2*np.dot(X1, X2.T) \
            + np.sum(np.square(X2), axis = 1) \
            + np.transpose([np.sum(np.square(X1), axis = 1)]))
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        # YOUR CODE HERE
        sorted_labels = y_train[np.argsort(dists[i])]
        closest_y = sorted_labels[:k]
        '''
        np.sum(k_labels==i) #统计k_labels中等于i的元素个数
        set(k_labels.flat)  #将b转为一维数组后，去除重复元素
        sorted()     #按元素个数从小到大排序
        [-1]        #取出元素个数最多的元组对 (count,element)
        [-1][1]    #取出最多的元素
        '''
        y_pred[i] = sorted([(np.sum(closest_y==i),i) for i in set(closest_y.flat)])[-1][1]
        # END YOUR CODE
...
def split_folds(X_train, y_train, num_folds):
    ...
    for i in range(num_folds):
        if (i+1)*validation_size <= X_train.shape[0]:
            X_vals[i,:,:] = X_train[i*validation_size:(i+1)*validation_size,:]
            y_vals[i,:] = y_train[i*validation_size:(i+1)*validation_size]
            X_trains[i,:,:] = np.delete(X_train,\
                                        np.arange(i*validation_size,(i+1)*validation_size),axis=0)
            y_trains[i,:] = np.delete(y_train,\
                                      np.arange(i*validation_size,(i+1)*validation_size),axis=0)
        else:
            X_vals[i,:,:] = X_train[i*validation_size:,:]
            y_vals[i,:] = y_train[i*validation_size:]
            X_trains[i,:,:] = X_train[:i*validation_size,:]
            y_trains[i,:] = y_train[:i*validation_size]
    # END YOUR CODE
