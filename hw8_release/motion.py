...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        # YOUR CODE HERE
        # 注意Ix_w对应Iy, Iy_w对应Ix
        Ix_w = Iy[y-w:y+w+1, x-w:x+w+1].reshape(-1, 1)
        Iy_w = Ix[y-w:y+w+1, x-w:x+w+1].reshape(-1, 1)
        A = np.c_[Ix_w, Iy_w]
        b = -It[y-w:y+w+1, x-w:x+w+1].reshape(-1, 1)
        d = np.dot(np.linalg.inv(A.T.dot(A)), A.T.dot(b)).reshape(-1)
        flow_vectors.append((d[0], d[1]))
        # END YOUR CODE
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        # YOUR CODE HERE
        Ix_w = Ix[y1-w:y1+w+1, x1-w:x1+w+1]  # 注意Ix_w对应Iy
        Iy_w = Iy[y1-w:y1+w+1, x1-w:x1+w+1]  # 注意Iy_w对应Ix
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        ...
        d = iterative_lucas_kanade(Il, Jl, pl, g=g)
        g = scale * (g + d)
        # END YOUR CODE
    d = g + d
    return d
...
def compute_error(patch1, patch2):
    ...
    # YOUR CODE HERE
    # Normalize patch1 and patch2
    normalized_p1 = (patch1 - np.mean(patch1)) / np.std(patch1)
    normalized_p2 = (patch2 - np.mean(patch2)) / np.std(patch2)
    # Compute mean square error between patch1 and patch2
    error = np.mean(np.square(normalized_p2-normalized_p1))
    # END YOUR CODE
    return error
...
def IoU(bbox1, bbox2):
    ...
    if x1 < x2:
        if y1 < y2:
            overlap = (w1 + x1 - x2) * (y1 + h1 - y2)
            score = overlap / (w1 * h1 + w2 * h2 - overlap)
            if score < 0:
                score = 0
        else:
            overlap = (w1 + x1 - x2) * (y2 + h2 - y1)
            score = overlap / (w1 * h1 + w2 * h2 - overlap)
            if score < 0:
                score = 0
    else:
        if y1 < y2:
            overlap = (w2 + x2 - x1) * (y1 + h1 - y2)
            score = overlap / (w1 * h1 + w2 * h2 - overlap)
            if score < 0:
                score = 0
        else:
            overlap = (w2 + x2 - x1) * (y2 + h2 - y1)
            score = overlap / (w1 * h1 + w2 * h2 - overlap)
            if score < 0:
                score = 0
